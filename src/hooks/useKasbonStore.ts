...
export function useKasbonStore(){
 ...
 const addDebt=useCallback((input:Omit<DebtItem,"id"|"remainingAmount"|"status"|"createdAt"|"paymentHistory">)=>{assertValidAmount(input.totalAmount);setDebts(v=>[{...input,id:newId(),remainingAmount:input.totalAmount,status:"UNPAID",createdAt:new Date().toISOString(),paymentHistory:[]},...v])},[]);
